...
module MakeSketchDomainRF (R: RANKING_FUNCTION)(P: PARTITION) : SketchDomainRF =
struct
...
let isLeq k t1 t2 =
  ...
  let rec config_filter t e =
    ...
    | A_bbinary (o,(e1,_),(e2,_)) ->
      ...
      (match o with
       | A_AND -> meet COMPUTATIONAL t1 t2
       | A_OR -> join COMPUTATIONAL t1 t2)
    | A_rbinary (_,_,_) ->
  ...
  let rec config_filter_constraint t e p1 =
	...
	let rec aux t bs (*constraints in expression e*) cs (*constraints collected up to the node*) =
      ...
      | [] ->
        (match t with
         | Leaf p -> Leaf (R.meet COMPUTATIONAL p p1) 
         | Node((c,nc),l,r) ->
           let bc = P.inner env_feats feats [c] in  (* Environmet features *)
           if (P.isLeq bcs bc)
           then (* c is redundant *) aux l bs cs
	...
	let rec aux t e cs =
        (match t with
         | Leaf p -> let b = match domain with | None -> P.inner env_feats feats cs | Some domain -> P.inner env_feats feats (cs@(P.constraints domain)) in
					 ...
					 let b'' = B.inner env_vars vars (P.constraints b') in 
		 			 let p' = R.filter ?domain:(Some b'') p e in 
					 Leaf p'
					 ...
					 if (P.isLeq b p'') then Leaf (P.meet p' b')
							...
							Node((!cs,ncs),Leaf (R.meet APPROXIMATION p' b'), Leaf (R.bot env_vars vars))
					 		) *)
        | Node ((c1,nc1),l1,r1) ->
  ...
  let rec print fmt t =
    ...
    let rec aux t cs =
      ...
      | Leaf p ->
		...
		let bb = match domain with | None -> B.inner env_feats feats cs | Some domain -> B.inner env_feats feats (cs@(P.constraints domain)) in 
		let bb' = B.project bb env_vars vars in 
        if P.isBot b then () else Format.fprintf fmt "[%a ?? %a] " P.print b R.print2 (bb',p)
      | Node((c,nc),l,r) -> aux r (nc::cs); aux l (c::cs)
  ...
  let rec print_terminate fmt t =
    ...
    let rec aux t cs =
      ...
      | Leaf p ->
		...
		let bb = match domain with | None -> B.inner env_feats feats cs | Some domain -> B.inner env_feats feats (cs@(P.constraints domain)) in 
		let bb' = B.project bb env_vars vars in 
        if P.isBot b then () else nn:=!nn+(R.print_terminate fmt env_feats feats bb' p)
      | Node((c,nc),l,r) -> aux r (nc::cs); aux l (c::cs)
    in aux t.tree []; !nn

  let rec result_terminate t =
    let domain = t.domain in
    let env_vars = t.env_vars in
    let env_feats = t.env_feats in	
    let vars = t.vars in 
    ...
    let rec aux t cs =
      ...
      | Leaf p ->
		...
		let bb = match domain with | None -> B.inner env_feats feats cs | Some domain -> B.inner env_feats feats (cs@(P.constraints domain)) in 
		let bb' = B.project bb env_vars vars in 
        if P.isBot b then () else tmp:=((R.result_terminate env_feats feats bb' p) @ !tmp)
      | Node((c,nc),l,r) -> aux r (nc::cs); aux l (c::cs)
    in aux t.tree []; !tmp
...
(** Decision Tree-based domain parameterized by the boxes numerical abstract domain. *)
 module SketchBRF = MakeSketchDomainRF(TSAB)(Numerical.B)
(*module SketchBORF = MakeSketchDomainRF(TSOB)(Numerical.B) *)
(** Decision Tree-based domain parameterized by the octagons abstract domain. *)
module SketchORF = MakeSketchDomainRF(TSAO)(Numerical.O)
(*module SketchOORF = MakeSketchDomainRF(TSOO)(Numerical.O) *)
(** Decision Tree-based domain parameterized by the polyhedra abstract domain. *)
module SketchPRF = MakeSketchDomainRF(TSAP)(Numerical.P)
(* module SketchPORF = MakeSketchDomainRF(TSOP)(Numerical.P) *)
