...
(*  module BanalApron = Banal_apron_domain.ApronDomain(N)  *)
  ...
  let remove_cons b var coeff = 
    let cs = ref [] in
    List.iter (fun c -> if not ((Coeff.equal (Lincons1.get_cst c) coeff) && (Coeff.equal_int (Lincons1.get_coeff c var) 1)) then cs := c::!cs) b.constraints;
	{ constraints = !cs; env = b.env; vars = b.vars }
  ...
  let fwdAssignSingle b (x,e) config = match x with
    | A_var x ->
      let env = b.env in
      let vars = b.vars in
      let e = Texpr1.of_expr env (aExpSingle_to_apron e config) in
      let a = Lincons1.array_make env (List.length b.constraints) in
      let i = ref 0 in
      List.iter (fun c -> Lincons1.array_set a !i c; i := !i + 1) b.constraints;
      let b = Abstract1.of_lincons_array manager env a in
    ...
    | _ -> raise (Invalid_argument "fwdAssignSingle: unexpected lvalue")
...
(*  let filter_underapprox (t:t) (e:bExp) : t = 
  ...
  let rec filterSingle b e config =
    match e with
    | A_TRUE -> b
    | A_MAYBE -> b
    | A_FALSE -> bot b.env b.vars
    | A_bunary (o,e) ->
      (match o with
       | A_NOT -> let (e,_) = negBExp e in filterSingle b e config)
    | A_bbinary (o,(e1,_),(e2,_)) ->
      let b1 = filterSingle b e1 config and b2 = filterSingle b e2 config in
      (match o with
       | A_AND -> meet b1 b2
       | A_OR -> join b1 b2)
    | A_rbinary (o,e1,e2) ->
      ...
      (match o with
       | A_LESS ->
         let e = Texpr1.of_expr env (aExpSingle_to_apron (A_abinary (A_MINUS,e2,e1)) config) in
         let c = Tcons1.make e Tcons1.SUP in
         let a = Tcons1.array_make env 1 in
         Tcons1.array_set a 0 c;
         let b = Abstract1.meet_tcons_array manager b a in
       ...
       | A_GREATER ->
         let e = Texpr1.of_expr env (aExpSingle_to_apron (A_abinary (A_MINUS,e1,e2)) config) in
         let c = Tcons1.make e Tcons1.SUP in
         let a = Tcons1.array_make env 1 in
         Tcons1.array_set a 0 c;
         let b = Abstract1.meet_tcons_array manager b a in
  ...
  let to_stringLatte b liveVars =
    ...
    | x::_ ->
      if (C.isBot x) then !lines else
        (let i = ref 1 and l = List.length !cs in
        List.iter (fun c ->
			(*C.print liveVars Format.std_formatter c; *)
			let l = C.to_stringLatte liveVars c in 
            lines := !lines @ [(l)];
			(*if (e) then eqs := !eqs @ [(!i)];*)
            i := !i + 1
          ) !cs; !lines)	
...
(*module EP = Numerical(
  struct 
    type lib = Elina_poly.loose Elina_poly.t
